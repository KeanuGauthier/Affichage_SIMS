...
markExplored(robotX, robotY, robotAngle) {
    if (!this.showExplored) return; // Si l'affichage des zones explorées est désactivé, quitter la fonction.

    const radius = 100; // Rayon maximal de vision.
    const halfAngle = 10; // Demi-angle du champ de vision.
    const rayCount = 40; // Nombre de rayons dans le champ de vision.
    const obstacleBuffer = 8; // Distance minimale pour considérer qu'un obstacle est rencontré.
    ...
    for (let i = 0; i <= rayCount; i++) {
        ...
        for (let dist = 0; dist <= radius; dist += this.cellSize) {
            ...
            // Arrêter si la position est en dehors du canvas.
            ...
            if (Obstacle.obstacles.some(obstacle => Math.hypot(obstacle.x - x, obstacle.y - y) <= obstacleBuffer)) {
                break; // Arrête le rayon dès qu'un obstacle est détecté.
            }
            ...
            if (!this.exploredCells.has(cellKey)) {
                this.exploredCells.add(cellKey); // Ajoute la cellule aux zones explorées.
                this.exploredArea += this.cellSize ** 2; // Augmente la surface explorée.
            }
        }
    }
}
...
updatePosition(targetX, targetY, targetAngle, vitesseDroit, vitesseGauche) {
    ...
    // Calcule la distance parcourue depuis la dernière position.
    if (this.previousX !== null && this.previousY !== null) {
        const dx = targetX - this.previousX;
        const dy = targetY - this.previousY;
        const distance = Math.sqrt(dx * dx + dy * dy);
        this.distanceParcourue += distance; // Met à jour la distance parcourue.

        this.map.markExplored(this.x, this.y, this.angle); // Met à jour les zones explorées.
    }

    // Ajoute la position actuelle au chemin si elle a changé.
    if (this.path.length === 0 || (this.previousX !== targetX || this.previousY !== targetY)) {
        this.path.push({ x: targetX, y: targetY });
    }

    this.previousX = targetX; // Met à jour la dernière position.
...
async function fetchData() {
    try {
        ...
        if (data.length > 0) {
            ...
            const { x, y, angle, distance0, distance1, distance2, distance3, distance4, vitesse_moteur_droit, vitesse_moteur_gauche } = lastMeasure;

            robot.updatePosition(x, y, angle, vitesse_moteur_droit, vitesse_moteur_gauche);

            const distances = [distance0, distance1, distance2, distance3, distance4];
            const angles = [0, 5, 10, -5, -10];

            distances.forEach((distance, index) => {
                if (distance < 100) {
                    const obstacleX = x + distance * Math.cos(((angle + angles[index]) * Math.PI) / 180);
                    const obstacleY = y + distance * Math.sin(((angle + angles[index]) * Math.PI) / 180);
